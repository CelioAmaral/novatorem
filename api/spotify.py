...
class SpotifyTokenManager:
    ...
    def _get_auth_header(self) -> str:
        ...
        credentials = (
            f"{spotify_config.client_id}:{spotify_config.client_secret}"
        )
...
# Audio features never change for a given track ID.
# Keeping them in memory avoids an additional Spotify request on every
# image generation.
_audio_features_cache: dict[str, Optional[AudioFeatures]] = {}
...
def get_recent_tracks(limit: int = 10) -> dict[str, Any]:
    ...
    url = (
        f"{spotify_config.recently_played_url}"
        f"?limit={limit}"
    )
...
def _extract_track_info(
    ...
    """
    Extract normalized track information from Spotify API response.
    """
...
def get_now_playing() -> dict[str, Any]:
    ...
    The current playback endpoint is intentionally checked every time
    so a song change is detected as quickly as possible.

    Expensive data such as audio features is cached by track ID.
    """
    ...
    try:
        data = _api_get(
            spotify_config.now_playing_url
        )
    ...
    if item is None:
        data = _api_get(
            f"{spotify_config.recently_played_url}?limit=1"
        )
        ...
        # Always use the most recent track.
        # The previous implementation selected a random track.
        item = items[0]["track"]
        is_playing = False
    ...
    track_info = _extract_track_info(
        item,
        is_playing,
    )
